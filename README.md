...
> contains 36 planted vulnerabilities (plus precision decoys) spanning the classes
...
> classes, and OWASP LLM10 denial-of-wallet via an uncapped agent-iteration
> budget). Difficulty runs from disguised
...
```bash
PYTHONPATH=. pytest -q                     # 8 functional + exploits (V01–V36) + 12 decoy checks
