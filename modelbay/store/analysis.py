"""Natural-language corpus analysis ("chat with your data").

A user asks a question in plain English; the assistant writes a short Python
snippet against the corpus's dataframe and we run it to produce the answer.
...
def run_analysis(question: str, rows: list[dict]) -> str:
    ...
    :func:`modelbay.assistant.backend.draft_code`); it is executed in a
    namespace scoped to the dataframe so the snippet can only touch ``df``.
    """
    from ..assistant.backend import draft_code

    columns = list(rows[0].keys()) if rows else []
    code = draft_code(question, columns=columns)
