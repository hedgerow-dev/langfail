...
STORAGE_DIR = Path(os.environ.get("LANGFAIL_STORAGE_DIR", BASE_DIR / "var" / "storage"))
...
class Config:
    SECRET_KEY = os.environ.get("LANGFAIL_SECRET_KEY", "langfail-dev-secret")
    JWT_SECRET = os.environ.get("LANGFAIL_JWT_SECRET", "langfail-dev-jwt-secret")
    JWT_ALGORITHM = "HS256"
    JWT_TTL_SECONDS = int(os.environ.get("LANGFAIL_JWT_TTL", "86400"))

    SQLALCHEMY_DATABASE_URI = os.environ.get(
        "LANGFAIL_DATABASE_URI", f"sqlite:///{(STORAGE_DIR / 'langfail.db').as_posix()}"
    )
    ...
    STRICT_PATHS = os.environ.get("LANGFAIL_STRICT_PATHS", "0") == "1"

    # Hosts the platform is allowed to reach when importing remote datasets/models.
    ALLOWED_FETCH_HOSTS = os.environ.get(
        "LANGFAIL_ALLOWED_FETCH_HOSTS", "huggingface.co,raw.githubusercontent.com,datasets.langfail.local"
    ).split(",")

    # LLM assistant backend: "stub" (deterministic, offline) or "ollama" (local server).
    LLM_BACKEND = os.environ.get("LANGFAIL_LLM_BACKEND", "stub")
    LLM_MODEL = os.environ.get("LANGFAIL_LLM_MODEL", "llama3.1")
    LLM_OLLAMA_URL = os.environ.get("LANGFAIL_LLM_OLLAMA_URL", "http://localhost:11434")
    ...
    MCP_HTTP_HOST = os.environ.get("LANGFAIL_MCP_HTTP_HOST", "0.0.0.0")
    MCP_HTTP_PORT = int(os.environ.get("LANGFAIL_MCP_HTTP_PORT", "8765"))
    MCP_HTTP_REQUIRE_AUTH = os.environ.get("LANGFAIL_MCP_HTTP_REQUIRE_AUTH", "0") == "1"
    MCP_HTTP_TOKEN = os.environ.get("LANGFAIL_MCP_HTTP_TOKEN", "")
